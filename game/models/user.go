...
type User struct {
	...
	Dbexpflag        int    `gorm:"column:dblexpflag;default:0"`    //是否处于自动战斗
	...
	Wg               string `gorm:"column:wg"`       //不知道是啥
...
type UserInfo struct {
	...
	TgtTime             int    `gorm:"column:tgttime;default:0"` // 上次通关、死亡时间
	TgtI                int    `gorm:"column:tgt_i;default:0"`
	GpcGroupId          int    `gorm:"column:gpc_group_id"`
	TgLastTime          int    `gorm:"column:tglasttime;default:0"` // 上次进入时间
	...
	ChouquChongwu       string `gorm:"column:chouqu_chongwu"`
...
func (u *UserInfo) AfterFind() (err error) {
	u.GetWelfareDate = com.StrTo(u.GetWelfareTime).MustInt()
	return
}
...
func (u *EmailMsg) TableName() string {
	return "information"
}
