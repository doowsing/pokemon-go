...
func NewRediStoreWithPool(pool *RedisClusterPool, keyPairs ...[]byte) (*RediStore, error) {
	rs := &RediStore{
...
func (s *RediStore) New(r *http.Request, name string) (*sessions.Session, error) {
	...
	if sessionId != "" {
		// 从c.Value中获取session.ID,可以考虑用base64
		if decoded, err := base64.StdEncoding.DecodeString(sessionId); err == nil {
			//fmt.Printf("get cookie val:%s, sessionid:%s\n", c.Value, decoded)
			session.ID = string(decoded)
...
func (s *RediStore) Save(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	...
	if session.Options.MaxAge <= 0 {
		...
		if session.ID == "" {
			session.ID = strings.TrimRight(base32.StdEncoding.EncodeToString(GenerateRandomKey(32)), "=")
		}
		if err := s.save(session); err != nil {
			return err
		}
		encoded := base64.StdEncoding.EncodeToString([]byte(session.ID))
		//encoded := session.ID

		fmt.Printf("save cookie val:%s, sessionid:%s\n", encoded, session.ID)
...
func NewStoreWithPool(pool *RedisClusterPool, keyPairs ...[]byte) (sessions.Store, error) {
...
func GetRedisStore(s sessions.Store) (err error, rediStore *RediStore) {
