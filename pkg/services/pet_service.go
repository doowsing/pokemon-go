...
type PetService struct {
	...
	rdrepo *rcache.PetRedisRepository
}

func NewPetService(osrc *OptService) *PetService {
	us := &PetService{repo: repositories.NewPetRepositories(), rdrepo: rcache.NewPetRedisRepository()}
	us.SetOptSrc(osrc)
	return us
}
...
func (ps *PetService) GetMskill(skillId int) *models.MSkill {
	mskill, err := ps.rdrepo.GetMSkill(skillId)
	if err != nil {
...
func (ps *PetService) GetMaxLevel(mpet *models.MPet) int {
	...
	if mpet.Wx != common.WxSS {
		...
		maxLevel = GetSSJhRule(mpet.ID).MaxLevel
	}
	return maxLevel
}
...
func (ps *PetService) PutOut(userId, petId int, inputpwd string) (result gin.H, msg string) {
	...
	user := ps.OptSrc.UserSrv.GetUserById(userId)
	if ps.OptSrc.UserSrv.CheckNeedPwd(inputpwd, user.McPwd) {
...
func (ps *PetService) Throw(userId, petId int, inputpwd string) (result gin.H, msg string) {
	...
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, 10000) {
	...
	if ps.GetDb().Delete(pet).RowsAffected > 0 {
		...
		ps.OptSrc.UserSrv.DecreaseJb(userId, -10000)
		msg = "操作失败！找不到该宠物！"
		return
	}
}
...
func (ps *PetService) DropPet(pet *models.UPet) bool {
	...
	if ps.GetDb().Delete(pet).RowsAffected > 0 {
		...
		ps.OptSrc.PropSrv.DropPetZb(pet.Uid, pet.ID)
...
func (ps *PetService) IncreaceExp2Pet(pet *models.UPet, exp int) bool {
	pet.GetM()
	...
	if Level > pet.Level {
		// 升级加属性
		grouth := GetGrowth(pet.MModel.Wx)

		czl := com.StrTo(pet.Czl).MustFloat64()
		pet.Hp += int(float64(grouth.Hp) * czl * float64(Level-pet.Level))
		pet.Mp += int(float64(grouth.Mp) * czl * float64(Level-pet.Level))
		pet.Ac += int(float64(grouth.Ac) * czl * float64(Level-pet.Level))
		pet.Mc += int(float64(grouth.Mc) * czl * float64(Level-pet.Level))
		pet.Hits += int(float64(grouth.Hits) * czl * float64(Level-pet.Level))
		pet.Miss += int(float64(grouth.Miss) * czl * float64(Level-pet.Level))
		pet.Speed += int(float64(grouth.Speed) * czl * float64(Level-pet.Level))
		pet.SrcHp = pet.Hp
		pet.SrcMp = pet.Mp
		ps.OptSrc.FightSrv.DelZbAttr(pet.ID)
	}
...
func (ps *PetService) IncreaceExp2MainPet(userId, exp int) *models.Response {
	usersvc := ps.OptSrc.UserSrv
	user := usersvc.GetUserById(userId)
	if user != nil {
		pet := ps.GetPet(user.ID, user.Mbid)
		if pet != nil {
			ok := ps.IncreaceExp2Pet(pet, exp)
			if ok {
				return &models.Response{Err: common.NewErr(200, "添加成功！"), Data: pet}
			} else {
				return models.NewNoDataResponse(common.ErrUsePropMaxLevel)
			}
		} else {
			return &models.Response{Err: common.NewErr(200, "参数出错"), Data: nil}
		}
	} else {
		return &models.Response{Err: common.NewErr(401, "请登陆后再访问！"), Data: nil}
	}
}
...
func (ps *PetService) LearnSkill(userId, skillPropId int) (bool, string) {
	prop := ps.OptSrc.PropSrv.GetProp(userId, skillPropId, false)
...
func (ps *PetService) UpdateSkill(userId, skillId int) (bool, string) {
	...
	if skill.MModel.Vary == "4" {
		...
		if !ps.OptSrc.PropSrv.DecrPropByPid(userId, 1666, 1) {
		...
		if !ps.OptSrc.PropSrv.DecrPropByPid(userId, 733, 1) {
...
func (ps *PetService) Evolution(userId, petId int, pathA bool, fzid int) (bool, string) {
	...
	lastTime, err := rcache.Hget("pet_evolution_time", strconv.Itoa(userId))
	if err != nil || len(lastTime) > 0 {
		lasttime := com.StrTo(lastTime).MustInt()
		now := ps.NowUnix()
		if lasttime+2 > now {
			return false, "进化冷却中！"
		}
	...
	if pathA {
		...
		usePropIds = apropIds
		apet := GetMpet(com.StrTo(petIds[0]).MustInt())
		if apet == nil {
		...
		if len(petIds) > 1 {
			bpet := GetMpet(com.StrTo(petIds[0]).MustInt())
			if bpet == nil {
				return false, "该宠物无法进化！"
			}
			topet = bpet
	...
	for i, _ := range usePropIds {
		prop := GetMProp(com.StrTo(usePropIds[i]).MustInt())
		if prop != nil && ps.OptSrc.PropSrv.DecrPropByPid(userId, prop.ID, 1) {
			finded = true
			useProp = prop
			break
		}
	...
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, 1000) {
		return false, "金币不足！"
	}
	newCzl := pet.CC
	...
	if pet.MModel.Wx < 6 {
		...
		if newCzl > 150 {
			if fzid != 0 {
				fzprop := ps.OptSrc.PropSrv.GetProp(userId, fzid, false)
				if fzprop == nil || !ps.OptSrc.PropSrv.DecrPropById(fzid, 1) {
	...
	ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("宠物进化：%s %s进化得到 %s，进化道具：%s", pet.MModel.Name, pathName, topet.Name, useProp.Name), 99)
	return true, "进化成功！"
}

func (ps *PetService) Emerge(userId, apetId, bpetId, apropId, bpropId int, noCheckZb, noCheckProtect bool) (result gin.H, msg string) {
	// 注意宠物要本人的，道具也要本人的
	result = gin.H{"result": false, "have_zb": false, "no_protect": false}
	...
	propNotes := []string{}
	if apropId != 0 {
		aprop := ps.OptSrc.PropSrv.GetProp(userId, apropId, false)
	...
	if bpropId != 0 {
		bprop := ps.OptSrc.PropSrv.GetProp(userId, bpropId, false)
	...
	ps.OptSrc.Begin()
	defer ps.OptSrc.Rollback()
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, 50000) {
		return result, "金币不足！"
	}
	if apropId != 0 && !ps.OptSrc.PropSrv.DecrPropById(apropId, 1) {
		return result, "添加物一数量不足！"
	}
	if bpropId != 0 && !ps.OptSrc.PropSrv.DecrPropById(bpropId, 1) {
		return result, "添加物二数量不足！"
	}
	user := ps.OptSrc.UserSrv.GetUserById(userId)
	...
	if successLuckeyNum <= successRate {
		...
		ps.OptSrc.UserSrv.SetMBid(userId, newPet.ID)
		ps.GetDb().Model(userInfo).Update(gin.H{"hecheng_nums": 0})
		ps.OptSrc.Commit()
		ps.OptSrc.SysSrv.SelfGameLog(userId,
		...
		ps.OptSrc.SysSrv.AnnouceAll(user.Nickname, fmt.Sprintf("成功的合成了一只%s,真是太幸运了!", toPet.Name))
		result["result"] = true
		...
		if !failedProtect {
			...
			if user.Mbid == bpet.ID {
				ps.OptSrc.UserSrv.SetMBid(userId, apetId)
			}
			ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("合成结果：失败！\n%s,\n消失副宠物：%s  level:%d  czl:%s  ac:%d  hits:%d",
...
func (ps *PetService) PetOffzb(userId, zbId int) (bool, string) {
	prop := ps.OptSrc.PropSrv.GetProp(userId, zbId, false)
	if prop == nil {
		return false, "道具不存在！"
	}
	if prop.Zbpets == 0 {
	...
	ps.OptSrc.Begin()
	defer ps.OptSrc.Rollback()
	if pet := ps.OptSrc.PetSrv.GetPet(userId, prop.Zbpets); pet != nil {
		newzbs := []string{}
...
func (ps *PetService) ZhuanSheng(userId, apetId, bpetId, cpetId, apropId, bpropId int) (bool, string) {
	...
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, 500000) {
		return false, "金币不足！"
	}
	if apropId != 0 && !ps.OptSrc.PropSrv.DecrPropById(apropId, 1) {
	...
	if luckeyNum <= int(float64(apet.Level+bpet.Level)/30*(1+addSuccessRate)*100) {
		...
		addCzlPetSettings1 := GetWelcome("zs1")
		addCzlPetSettings2 := GetWelcome("zs2")
		addCzlPetSettings3 := GetWelcome("zs3")
		var num1, num2 float64 = 0, 0
		if addCzlPetSettings1 != nil && com.IsSliceContainsStr(strings.Split(addCzlPetSettings1.Content, ","), strconv.Itoa(apetId)) {
			// 没有加成的
			if apet.CC <= 10.9 {
				num1, num2 = 1, 2200
		...
		} else if addCzlPetSettings2 != nil && com.IsSliceContainsStr(strings.Split(addCzlPetSettings2.Content, ","), strconv.Itoa(apetId)) {
			// 有小加成的
			if apet.CC <= 10.9 {
				num1, num2 = 1, 2190
			} else if apet.CC <= 30.9 {
		...
		} else if addCzlPetSettings3 != nil && com.IsSliceContainsStr(strings.Split(addCzlPetSettings3.Content, ","), strconv.Itoa(apetId)) {
			// 有大加成的
			if apet.CC <= 10.9 {
				num1, num2 = 1, 2180
			} else if apet.CC <= 30.9 {
		...
		addCzl := (float64(apet.Level)/apet.CC/num1 + float64(bpet.Level)*bpet.CC/num2) * addCzlRate
		newPet.Czl = utils.CzlStr(apet.CC + addCzl)
		...
		user := ps.OptSrc.UserSrv.GetUserById(userId)
		ps.OptSrc.SysSrv.AnnouceAll(user.Nickname, fmt.Sprintf("成功的转生获得了一只%s,真是太幸运了!", toPet.Name))
...
func (ps *PetService) Chouqu(userId, petId, apid, bpid int) (bool, string) {
	...
	propNote := []string{}
	if apid != 0 {
		aprop := ps.OptSrc.PropSrv.GetProp(userId, apid, false)
		if aprop == nil || aprop.Sums == 0 {
			return false, "添加物一数量不足！"
		}
		aprop.GetM()
	...
	if bpid != 0 {
		bprop := ps.OptSrc.PropSrv.GetProp(userId, bpid, false)
		if bprop == nil || bprop.Sums == 0 {
			return false, "添加物二数量不足！"
		}
		bprop.GetM()
	...
	if apid != 0 && !ps.OptSrc.PropSrv.DecrProp(userId, apid, 1) {
		return false, "添加物一数量不足！"
	}
	if bpid != 0 && !ps.OptSrc.PropSrv.DecrProp(userId, bpid, 1) {
		return false, "添加物二数量不足！"
	}
	...
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, cqJb) {
		return false, "金币不足！"
	}
	...
	ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("抽取成长：被抽取的宠物 %s (id=%d 成长:%s),抽取了:%d成长,使用物品:%s",
...
func (ps *PetService) Zhuanhua(userId, petId, czl int) (bool, string) {
	...
	ssRule := GetSSJhRule(pet.MModel.ID)
	if ssRule == nil {
	...
	ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("神圣成长转化：转化%d成长给%s", czl, pet.MModel.Name), 103)
	return true, "转化成长成功！"
}
...
func (ps *PetService) SSEvolution(userId, petId, fzid int) (bool, string) {
	...
	ssjhRule := GetSSJhRule(pet.MModel.ID)
	if ssjhRule == nil {
		return false, "该宠物无法进化！"
	}
	logNote := "神圣进化："
	...
	ps.OptSrc.Begin()
	defer ps.OptSrc.Rollback()
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, jb) {
		return false, "金币不足！"
	}
	if !ps.OptSrc.PropSrv.DecrPropByPid(userId, propId, num) {
		return false, "进化道具不足！"
	}
	attrType := ""
	attrNum := 0
	if fzid != 0 {
		fzprop := ps.OptSrc.PropSrv.GetProp(userId, fzid, false)
		if fzprop == nil || fzprop.Sums == 0 || !ps.OptSrc.PropSrv.DecrProp(userId, fzprop.ID, 1) {
	...
	growth := GetGrowth(pet.MModel.Wx)
	...
	if ps.GetDb().Model(pet).Where("level>1").Update(gin.H{
		...
		ps.OptSrc.Commit()
		ps.OptSrc.FightSrv.DelZbAttr(pet.ID)
		ps.OptSrc.SysSrv.SelfGameLog(userId, logNote, 103)
		return true, "进化成功！"
	} else {
...
func (ps *PetService) SSZhuanshengInfo(userId, petId int) []gin.H {
	...
	for _, rule := range sszsRule {
		...
		for _, str := range strings.Split(rule.NeedProps, ",") {
			items := strings.Split(str, "|")
			if len(items) > 1 {
				_prop := GetMProp(com.StrTo(items[0]).MustInt())
				needProps = append(needProps, fmt.Sprintf("%s x %s", _prop.Name, items[1]))
			}
		}
		jhRule := GetSSJhRule(topet.ID)
...
func (ps *PetService) SSZhuanSheng(userId, petId, topetId, apropId, bpropId int) (bool, string) {
	...
	if pet.CC < com.StrTo(sszsRule.NeedCzl).MustFloat64() {
		return false, "成长需求不足！"
	}
	if pet.Level < com.StrTo(sszsRule.NeedLevel).MustInt() {
		return false, "等级需求不足！"
	}
	...
	jhRule := GetSSJhRule(topetId)
	if jhRule == nil {
		return false, "该神圣宠物无法转生！"
	}
	needJb := jhRule.ZsProgress * 100000
	if !ps.OptSrc.UserSrv.DecreaseJb(userId, needJb) {
		return false, "金币需求不足！"
	}
	needProps := []string{}
	for _, str := range strings.Split(sszsRule.NeedProps, ",") {
		...
		if len(items) > 1 {
			...
			if !ps.OptSrc.PropSrv.DecrPropByPid(userId, pid, num) {
	...
	addProps := []string{}
	if apropId != 0 {
		prop := ps.OptSrc.PropSrv.GetProp(userId, apropId, false)
		if prop == nil || prop.Sums == 0 || !ps.OptSrc.PropSrv.DecrProp(userId, apropId, 1) {
	...
	if bpropId != 0 {
		prop := ps.OptSrc.PropSrv.GetProp(userId, bpropId, false)
		if prop == nil || prop.Sums == 0 || !ps.OptSrc.PropSrv.DecrProp(userId, bpropId, 1) {
	...
	if utils.RandInt(1, 10000) <= successNum {
		...
		if ps.CreatPet(newPet) {
			ps.DropPet(pet)
			newPet.GetM()
			ps.OptSrc.SysSrv.AnnouceAll(ps.OptSrc.UserSrv.GetUserById(userId).Nickname, "获得神圣宠物 "+newPet.MModel.Name)
			ps.OptSrc.UserSrv.SetMBid(userId, newPet.ID)
			ps.OptSrc.Commit()

			ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("神圣转生：成功！获得 %s, 消耗物品：%s, 辅助道具：%s, \n原宠物：%s, czl: %s, ac:%d, hits:%d\n, 新宠物：%s, czl: %s, ac:%d, hits:%d\n",
		...
		ps.OptSrc.Commit()
		ps.OptSrc.SysSrv.SelfGameLog(userId, fmt.Sprintf("神圣转生：失败！消耗物品：%s, 辅助道具：%s",
